...
\label{alg:SI}
\begin{algorithmic}[1]
\STATE \textbf{Input:} Swarm population size $\textit{n}$, Set of stopping criteria $\lambda\textsubscript{i}$, fitness function $\textit{f}$
\STATE Create a Swarm of candidate solutions $\textit{S}$ of size $\textit{n}$ 
...
Under the umbrella of EA, Genetic Algorithms (GA) has emerged as one of the most popular search and optimization techniques. The core of GA lies in the feedback process, which updates the candidate solutions in accordance with 3 main sub-processes. Figure $\ref{ga}$ summarizes the entire process of GA.

\begin{itemize}
\item Selection: This step involves selecting the candidates having the best fitness score 
\item Crossover: The characteristics of the parents selected at the selection stage are intermixed to generate better candidates in the next generation 
\item Mutation: As an add-on to the generational transition, random changes are added to add diversity and possibly improve the candidates
\end{itemize}
...
\begin{figure}[!t]
\centering
\includegraphics[height=6cm]{struct.png}
...
The entire process of guiding and voting is displayed in Algorithm $\ref{alg:da}$.

\begin{algorithm}[!t]
\footnotesize
\caption{DA pseudocode}
\label{alg:da}
\begin{algorithmic}[1]
...
In this section the DA algorithm is benchmarked against 18 standard minimization functions. These functions are described in Table $\ref{tab:1}$. The DA was iterated 20 times on each of these functions and the results were compared against other popular SI algorithms such as Particle Swarm Optimization (PSO), Grey Wolf Optimization (GWO), Multiverse Optimization (MVO) and Cuckoo Search (CS). The metric based results such as mean and standard deviation is listed in Table $\ref{tab:2}$. The experiment was set up on a 64-bit Machine with Ubuntu 20.04, Intel® CoreTM i5-7200U CPU @ 2.50GHz × 4, with a GeForce 940MX/PCle/SSE2 graphic card.

Based on the results, DA provides ambitious results on the becnhmark functions. DA outperforms the other algorithms in F2, F8, F15 and F18. The selective performance boosts could be attributed to the optimal voting season. In certain cases the top departments would have saturated and the replacement process might have been ineffective. As it is the case with any algorithm, global optimum search can never be guarenteed.  
...
\scalebox{0.9}
{
\begin{tabular}{| >{\arraybackslash}m{0.88in} | c | c | >{\arraybackslash}m{0.4in} | c | >{\arraybackslash}m{1in} |}
\hline
Function & ID & Formula & Modality & Range & Minimum Value \\
\hline
Matyas & F1 & $0.26(x\textsubscript{2} + y\textsubscript{2}) - 0.48xy$ & Uni & [-10,10] & 0 at $x\textsuperscript{*}$ = (0,0)\\
Booth & F2 & $(x + 2y - 7)\textsuperscript{2} + (2x + y - 5)\textsuperscript{2}$ & Uni & [-10,10] & 0 at $x\textsuperscript{*}$ = (1,3)\\
Bohachevsky & F3 & $x\textsuperscript{2} + 2y\textsuperscript{2} -0.3cos(3\pi x)-0.4cos(4\pi y)+0.7$ & Uni & [-100,100] & 0 at $x\textsuperscript{*}$ = (0,0)\\
Gramacy and Lee & F4 & $\frac{sin(10\pi x)}{2x} + (x-1)\textsuperscript{4}$ & Uni & [-0.5,2.5] & -0.869011134989500 at $x\textsuperscript{*}$ = (0.548563444114526)\\
Leon & F5 & $100(y-x\textsuperscript{3})\textsuperscript{2}$ + $(1-x)\textsuperscript{2}$ & Uni & [-10,10] & 0 at $x\textsuperscript{*}$ = (1,1)\\
Ackley & F6 & $-20 exp(-0.2 \sqrt{\frac{1}{n} \sum_{i=1}^n x_i^2}) - exp(\frac{1}{n} \sum_{i=1}^n cos(2\pi x_i)) + 20 + e$ & Multi & [-32,32] & 0 at $x\textsuperscript{*}$ = (0,0)\\
Bartels & F7 & $|x\textsuperscript{2} + y\textsuperscript{2} + xy| + |sin(x)| + |cos(y)|$ & Multi & [-500,500], [-500,500] & 1 at $x\textsuperscript{*}$ = (0,0)\\
Branin & F8 & $a(x\textsubscript{2} - bx\textsubscript{1}\textsuperscript{2} + cx\textsubscript{1} - r)\textsuperscript{2}$ + $s(1-t)cos(x\textsubscript{1})$ + s & Multi & [-5,10], [0,15] & 0.397887 at $x\textsuperscript{*}$ = (-$\pi$, 12.2.275)\\
Egg Crate & F9 & $x\textsuperscript{2}$ + $y\textsuperscript{2}$ + $25(sin\textsuperscript{2}(x) + cos\textsuperscript{2}(x))$ & Multi & [-5,5] & 0 at $x\textsuperscript{*}$ = (0,0)\\
Qing & F10 & $\sum_{i=1}^{n}(x^2-i)^2$ & Multi & [-500,500] & 0 at $x\textsuperscript{*}$ = ($\pm\sqrt{i}$)\\
Rastrigin & F11 & $10n + \sum_{i=1}^{n}(x_i^2 - 10cos(2\pi x_i))$ & Multi & [-5.12,5.12], [-500,500] & 0 at $x\textsuperscript{*}$ = (0,0)\\
Rosenbrock & F12 & $\sum_{i=1}^{n}[b (x_{i+1} - x_i^2)^ 2 + (a - x_i)^2]$ & Multi & [-5.12,5.12], [-5,10] & 0 at $x\textsuperscript{*}$ = (1)\\
Bird & F13 & $sin(x)e\textsuperscript{(1-cos(y))^2}+cos(y)e\textsuperscript{(1-sin(x))^2}+(x-y)\textsubscript{2}$ & Multi & [-2$\pi$, 2$\pi$] & -106.764537 at $x\textsuperscript{*}$ = (4.70104,3.15294), (-1.58214,-3.13024)\\
Powell Sum & F14 & $\sum_{i=1}^{n}|x_i|^{i+1}$ & Multi & [-1,1] & 0 at $x\textsuperscript{*}$ = 0\\
Schaffer & F15 & $0.5 + \frac{sin\textsuperscript{2}(x\textsuperscript{2}-y\textsuperscript{2})-0.5}{(1+0.001(x\textsuperscript{2}+x\textsuperscript{2}))\textsuperscript{2}}$ & Multi & [-100,100] & 0 at $x\textsuperscript{*}$ = (0,0)\\
Shubert & F16 & $\prod_{i=1}^{n}{\left(\sum_{j=1}^5{ cos((j+1)x_i+j)}\right)}$ & Multi & [-10,10] & -186.7309 \\
Happy Cat & F17 & $\left[\left(||\textbf{x}||^2 - n\right)^2\right]^\alpha + \frac{1}{n}\left(\frac{1}{2}||\textbf{x}||^2+\sum_{i=1}^{n}x_i\right)+\frac{1}{2}$ & Multi & [-2,2] & 0 at $x\textsuperscript{*}$ = (-1) \\
Alpine & F18 & $\prod_{i=1}^{n}\sqrt{x_i}sin(x_i)$ & Multi & [0,10] & 2.808 at $x\textsuperscript{*}$ = (7.917) \\
\hline
\end{tabular}
}
\end{table*}
...
The plots for F3 and F6 have been shown in Figures $\ref{fig:sfig1}$ and $\ref{fig:sfig2}$. For these two functions, the initial top department was capable of producing acceptable results. Due to the acceptance, the voting season never arrived and the process halted. 
...
The plots for F8 and F9 have been shown in Figures $\ref{fig:sfig3}$ and $\ref{fig:sfig6}$. For these two functions, the initial top department was inapable of producing acceptable results. In the following voting season the total reward did not exceed $\lambda$ and hence B dominance started. 
...
\item Work out multiple function inputs parallely.
\end{itemize}

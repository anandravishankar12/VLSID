...
\begin{abstract}
Computational intelligence has produced nature inspired algorithms as the new and popular problem solving tool in Multi-Objective Optimization Problems (MOOPs). In this paper we present Democratic Algorithm (DA), a social-construct based algorithm heavily inspired from hierarchical swarm intelligence and real life social establishments. Each candidate in the swarm takes part in the search process and contends for the position of the leader. Additional enhancements such as dynamism and memory retention capabilities boost the performance of DA. A comprehensive study is conducted with 18 benchmark functions to act as proof-of-concept and lay the groundwork for future works. The results so obtained, cement the competence of DA as it outperforms other algorithms in multiple unimodal and multimodal landscapes.
...
In this section we present a novel social-construct based algorithm heavily inspired from swarm intelligence and real life social establishments. In order to find the global Pareto Optimal solution, each candidate aims to transcend to the next department with the help of the previous generations. 
...
Let $X$ = $\{X\textsuperscript{1}, X\textsuperscript{2} \dots, X\textsuperscript{n} \}$ be the candidates and $a\textsubscript{i}$ and $b\textsubscript{i}$ be the candidate and solutino incidence vectors. \\
\begin{align*}
...
The implication of $d(F\textsubscript{t}, M\textsuperscript{*}) \rightarrow 0$ implies $\delta\textsubscript{M\textsuperscript{*}}(F\textsubscript{t}) \rightarrow 0$ extends to the worst case scenario of $n = M\textsuperscript{*}$. However, if a Markov Chain is maintained dynamically, $|M\textsuperscript{*}|=1$ and the limited population size will stagnate to a critical point. 
